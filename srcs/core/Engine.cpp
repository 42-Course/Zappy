...
Engine::Engine(const Config &config)
    : world_(std::make_unique<World>(config.getMapWidth(), config.getMapHeight(),
                                     config.isInfiniteMap())),
      gameLoop_(std::make_unique<GameLoop>(config.getTickRate())),
      network_(std::make_unique<NetworkManager>(*world_, *gameLoop_, config.getPlayerPort(),
                                                config.getSpectatorPort())),
      watchService_(std::make_unique<WatchService>(*this)), running_(false) {
...
void Engine::registerCommands() {
  ...
  network_->registerServerCommand(
    "help", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<HelpCommand>(*this, tokens, client);
    });

  network_->registerServerCommand(
    "status", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<StatusCommand>(*this, tokens, client);
    });

  network_->registerServerCommand(
    "exit", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<ExitCommand>(*this, tokens, client);
    });

  network_->registerServerCommand(
    "watch", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<WatchCommand>(*this, tokens, *watchService_, client);
    });

  network_->registerServerCommand(
    "clear", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<ClearCommand>(*this, tokens, client);
    });

  // Register spectator command handlers
  network_->registerSpectatorCommand(
    "msz", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<MSZCommand>(*world_, tokens, client);
    });

  network_->registerSpectatorCommand(
    "bct", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<BCTCommand>(*world_, tokens, client);
    });

  network_->registerSpectatorCommand(
    "mct", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<MCTCommand>(*world_, tokens, client);
    });

  network_->registerSpectatorCommand(
    "tna", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<TNACommand>(*world_, tokens, client);
    });

  network_->registerSpectatorCommand(
    "ppo", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<PPOCommand>(*world_, tokens, client);
    });

  network_->registerSpectatorCommand(
    "plv", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<PLVCommand>(*world_, tokens, client);
    });

  network_->registerSpectatorCommand(
    "pin", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<PINCommand>(*world_, tokens, client);
    });

  network_->registerSpectatorCommand(
    "sgt", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<SGTCommand>(*gameLoop_, tokens, client);
    });

  network_->registerSpectatorCommand(
    "sst", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<SSTCommand>(*gameLoop_, tokens, client);
    });

  // Register player command handlers
  network_->registerPlayerCommand(
    "forward", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<ForwardCommand>(*world_, tokens, client);
    });

  network_->registerPlayerCommand(
    "right", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<RightCommand>(*world_, tokens, client);
    });

  network_->registerPlayerCommand(
    "left", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<LeftCommand>(*world_, tokens, client);
    });

  network_->registerPlayerCommand(
    "inventory", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<InventoryCommand>(*world_, tokens, client);
    });

  network_->registerPlayerCommand(
    "see", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<SeeCommand>(*world_, tokens, client);
    });

  network_->registerPlayerCommand(
    "take", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
      return std::make_unique<TakeCommand>(*world_, tokens, client);
    });

  network_->registerPlayerCommand(
    "drop", [this](const std::vector<std::string> &tokens, ClientConnection *client) {
