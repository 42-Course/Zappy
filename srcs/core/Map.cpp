...
Map::Map(int width, int height, bool infinite)
    : width_(width), height_(height), infinite_(infinite) {
  if (width <= 0 || height <= 0) {
...
void Map::notifyResourceAdded(const Tile *tile, ResourceType type) const {
  ...
  notify([tile, type](IObserver *obs) { obs->onResourceAdded(tile, type); });
}
...
void Map::notifyResourceRemoved(const Tile *tile, ResourceType type) const {
  ...
  notify([tile, type](IObserver *obs) { obs->onResourceRemoved(tile, type); });
}
