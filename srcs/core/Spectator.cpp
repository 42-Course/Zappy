...
std::string Spectator::getPlayerPosition(int playerId) const {
  if (!world_)
    return "ppo " + std::to_string(playerId) + " -1 -1 0\n";
  const auto *player = world_->getPlayer(playerId);
  if (!player)
    return "ppo " + std::to_string(playerId) + " -1 -1 0\n";
  return "ppo " + std::to_string(playerId) + " " + std::to_string(player->getX()) + " " +
...
std::string Spectator::getPlayerLevel(int playerId) const {
  if (!world_)
    return "plv " + std::to_string(playerId) + " 0\n";
  const auto *player = world_->getPlayer(playerId);
  if (!player)
    return "plv " + std::to_string(playerId) + " 0\n";
...
std::string Spectator::getPlayerInventory(int playerId) const {
  if (!world_)
    return "pin " + std::to_string(playerId) + " -1 -1 0 0 0 0 0 0 0\n";
  const auto *player = world_->getPlayer(playerId);
  if (!player)
    return "pin " + std::to_string(playerId) + " -1 -1 0 0 0 0 0 0 0\n";
  // TODO: Implement actual inventory querying
  return "pin " + std::to_string(playerId) + " " + std::to_string(player->getX()) + " " +
