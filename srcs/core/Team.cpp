...
namespace Zappy {
Team::Team(const std::string &name, int maxPlayers) : name_(name), maxPlayers_(maxPlayers) {
  if (maxPlayers <= 0) {
...
void Team::notifyPlayerAdded(const Player *player) const {
  notify([player](IObserver *obs) { obs->onPlayerAdded(player); });
}

void Team::notifyPlayerRemoved(const Player *player) const {
  notify([player](IObserver *obs) { obs->onPlayerRemoved(player); });
}

void Team::notifyWinCondition() const {
  notify([this](IObserver *obs) { obs->onTeamWon(this); });
}
...
void Team::update() {
  ...
  auto it = std::remove_if(players_.begin(), players_.end(),
                           [](const Player *player) { return player && !player->isAlive(); });
