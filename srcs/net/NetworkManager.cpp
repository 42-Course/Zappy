...
NetworkManager::NetworkManager(World &world, GameLoop &gameLoop, int playerPort, int spectatorPort)
    : world_(world), gameLoop_(gameLoop), playerSocket_(AF_INET, SOCK_STREAM, 0),
      spectatorSocket_(AF_INET, SOCK_STREAM, 0), playerPort_(playerPort),
      spectatorPort_(spectatorPort), broadcaster_(clientManager_), running_(false) {
  ...
  try {
    ...
    if (bind(spectatorSocket_.getFd(), (struct sockaddr *)&spectatorAddr, sizeof(spectatorAddr)) ==
        -1) {
...
void NetworkManager::acceptNewConnection(int serverFd, ClientConnection::Type type) {
  ...
  while (true) {
    ...
    eventLoop_.addFd(clientFd, EPOLLIN | EPOLLOUT | EPOLLET,
                     [this, clientFd](uint32_t events) { handleClientEvent(clientFd, events); });
  }
}
...
NetworkManager::createServerCommand(const std::string &command,
