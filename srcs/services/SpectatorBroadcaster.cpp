...
void SpectatorBroadcaster::onPlayerLevelUp(const Player *player) {
  if (!player)
    return;
  std::string msg =
    "plv " + std::to_string(player->getId()) + " " + std::to_string(player->getLevel()) + "\n";
  broadcast(msg, true);
}
...
void SpectatorBroadcaster::onPlayerInventoryChanged(const Player *player) {
  ...
  std::string msg =
    "pin " + std::to_string(player->getId()) + " " + std::to_string(player->getX()) + " " +
    std::to_string(player->getY()) + " " + std::to_string(inv.getFood()) + " " +
    std::to_string(inv.getLinemate()) + " " + std::to_string(inv.getDeraumere()) + " " +
    std::to_string(inv.getSibur()) + " " + std::to_string(inv.getMendiane()) + " " +
    std::to_string(inv.getPhiras()) + " " + std::to_string(inv.getThystame()) + "\n";
  broadcast(msg, true);
}
...
void SpectatorBroadcaster::onPlayerAdded(const Player *player) {
  if (!player)
    return;
  std::stringstream ss;
  ss << "pnw " << player->getId() << " " << player->getX() << " " << player->getY() << " "
     << static_cast<int>(player->getDirection()) << " " << player->getLevel() << " "
...
std::string SpectatorBroadcaster::formatTileMessage(const Tile *tile, int x, int y) {
  if (!tile)
    return "";
  std::stringstream ss;
  ss << "bct " << x << " " << y << " " << tile->getResourceCount(ResourceType::FOOD) << " "
