...
WatchService::WatchService(Engine &engine) : _engine(engine), _enabled(false) {}
...
void WatchService::printServerInfo() const {
  ...
  double totalSeconds = static_cast<double>(_engine.getGameLoop().getCurrentTick()) /
                        _engine.getGameLoop().getTickRate();
